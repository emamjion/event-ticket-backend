...
const generateOrderTicketPDF = async (order, event) => {
  return new Promise(async (resolve, reject) => {
    ...
    // If no seats, fallback to single "null" seat for generating 1 ticket
    const seats = order?.seats?.length ? order.seats : [null];

    for (let i = 0; i < seats.length; i++) {
      const seat = seats[i];

      // Get ticket code for this seat, fallback if not array
      const ticketCode =
        (Array.isArray(order.ticketCodes) && order.ticketCodes[i]) ||
        order.ticketCode ||
        "000000000000";
      ...
      if (barcodeBuffer) {
        try {
          ...
          console.error("Error adding barcode image:", imgError);
          doc.fillColor("#2d3748").fontSize(10).font("Courier");
          doc.text(ticketCode, rightColX + 20, rightY + 22);
        }
      } else {
        // Manual barcode pattern fallback
        doc.fillColor("#2d3748");
        ...
        for (
          let j = 0;
          j < ticketCode.length && barX < rightColX + barcodeWidth - 20;
          j++
        ) {
          const digit = parseInt(ticketCode[j]) || 0;
          const pattern = digit % 4;
